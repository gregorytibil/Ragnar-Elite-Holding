      ...
      <div className="max-w-[1140px] mx-auto px-6 pb-24 md:pb-36 relative z-10 -mt-12 md:-mt-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24 relative">
          ...
          <div className="lg:col-span-4 hidden lg:block">
            <div className="sticky top-32 space-y-8 bg-white/50 backdrop-blur-md p-8 border border-[#0B1B3D]/10 rounded-sm">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start gap-3 pt-4 border-t border-[#0B1B3D]/10">
                    <span className="font-mono text-[10px] text-[#A2D2FF] font-bold mt-1">07</span>
                    <span className="text-sm font-sans font-medium text-[#0B1B3D] uppercase tracking-wide leading-snug">
                       {t.legalSec7Title}
                    </span>
                </div>
          ...
          >
            {sections.map((sec, idx) => (
              <div key={idx} className="group relative">
                ...
                <h2 className="text-2xl md:text-3xl font-sans font-bold uppercase tracking-tight text-[#0B1B3D] mb-6 flex items-start gap-4">
                  <span className="text-[#A2D2FF] mt-1.5 lg:hidden"><FileText className="w-6 h-6" /></span>
                  {sec.title}
                </h2>
            ...
            <div className="group relative border-t border-[#0B1B3D]/10 pt-16 lg:pt-24">
              <div className="hidden lg:block absolute -left-12 top-17 font-mono text-sm text-[#A2D2FF]/50 font-bold">
                07
              </div>
              <h2 className="text-2xl md:text-3xl font-sans font-bold uppercase tracking-tight text-[#0B1B3D] mb-8 flex items-start gap-4">
                  <span className="text-[#A2D2FF] mt-1.5 lg:hidden"><Scale className="w-6 h-6" /></span>
                {t.legalSec7Title}
              </h2>
              <div className="space-y-6">
                {[t.legalSec7Bullet1, t.legalSec7Bullet2, t.legalSec7Bullet3, t.legalSec7Bullet4].map((bullet, i) => (
                    <div key={i} className="flex gap-4 p-6 bg-white border border-[#0B1B3D]/5 shadow-sm rounded-sm">
                        <span className="text-[#A2D2FF] mt-1.5 shrink-0">—</span>
                        <p className="text-base text-slate-700 font-light leading-relaxed">
                            {bullet.includes(':') ? (
                                <>
                                    <strong className="font-semibold text-[#0B1B3D]">{bullet.split(':')[0]}:</strong> {bullet.split(':')[1]}
                                </>
                            ) : bullet}
                        </p>
                    </div>
              ...
              <div className="mt-12 p-8 md:p-12 bg-[#050D1D] text-[#FAF6F0] rounded-sm relative overflow-hidden">
                ...
                <Quote className="w-8 h-8 text-[#A2D2FF] mb-6 relative z-10" />
